...
if ($method == 'POST') {
        ...
        }else if (empty($cointrackid)) {//checking if data is empty
            $errordesc="Bad request";
            $linktosolve="https://";
            ...
            if(isset($_POST['search'])&&$_POST['search']!=''){
              ...
              for($i=1;$i<=2;$i++){
                 ...
                 $bindParams[]=$search;
                        ...
                        $active=1;
                        $checksubwall =  $connect->prepare("SELECT * FROM swap_system_settings WHERE coin_frm_trackid=?  AND status=? $searchclause");
                        ...
                        if ($dsubwallresult->num_rows >0) {
                            while($coinsdata =$dsubwallresult->fetch_assoc()){
                                ...
                                $coinsdata['balance']="$towalletbalancis";
