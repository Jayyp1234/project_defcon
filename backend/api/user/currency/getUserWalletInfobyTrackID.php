...
if ($method == 'GET') {
    ...
    $user_pubkey = cleanme($decodedToken->usertoken);
        ...
        if (!isset($_GET['tag'])){
            $errordesc="Bad request";
            $linktosolve="https://";
            $hint=["Tag Params Not Set","Ensure that all data sent is not empty","Ensure that the exact data type specified in the documentation is sent."];
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            ...
            $track_id = cleanme($_GET['tag']);
            $active=1;
            $sqlQuery = "SELECT * FROM userwallet INNER JOIN currencysystem ON userwallet.currencytag= currencysystem.currencytag WHERE currency_status = ? AND userid = ? AND wallettrackid = ?";
            $stmt= $connect->prepare($sqlQuery);
            $stmt->bind_param("sss",$active,$userid,$track_id);
            $stmt->execute();
            $result= $stmt->get_result();
            ...
            if($numRow > 0){
                ...
                while($users = $result->fetch_assoc()){
                    ...
                    $sqlQuery = "SELECT walletbal,coinsystrackid,walletpendbal,coinsystemtag FROM usersubwallet WHERE userid=? AND usersubwallet.currencytag = ? ";
                  ...
                  if($numRow2 > 0){
                    ...
                    $allResponse = array("totalbal"=>$totalbalanceis,"totalpendbal"=>$totalpendbal,"id"=>$users['id'], "userid"=>$users['userid'], "currencytag"=>$users['userid'], "wallettrackid"=>$users['wallettrackid'], "walletbal"=>round($users['walletbal'],2), "walletpendbal"=>round($users['walletpendbal'],2), "walletescrowbal"=>round($users['walletescrowbal'],2),
