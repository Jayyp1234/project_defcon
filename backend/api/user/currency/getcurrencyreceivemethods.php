...
if ($method == 'GET') {
    ...
    $user_pubkey = cleanme($decodedToken->usertoken);
        ...
        if (!isset ($_GET['currency']) ) {  
            ...
            $currencyClause = "AND currencyreceivemethods.currencytag = ?";
        }
        
        
        $active=1;
        $mainorsubwallet=1;
        $sqlQuery = "SELECT * FROM currencyreceivemethods WHERE status=?  AND mainorsubwallet=? ".$currencyClause ." ORDER BY id DESC";
        $stmt= $connect->prepare($sqlQuery);
        if (isset ($_GET['currency'])) { 
            $stmt->bind_param("sss",$active,$mainorsubwallet,$currecncytagis);
        }else{
            $stmt->bind_param("ss",$active,$mainorsubwallet);
        }
