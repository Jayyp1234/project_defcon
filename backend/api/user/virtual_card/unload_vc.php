...
if (getenv('REQUEST_METHOD') === 'POST'){
        ...
        $active=1;
        $checkdata =  $connect->prepare("SELECT customer_id,vc_type_tid,wallet_id,vc_card_id FROM  vc_customer_card WHERE trackid=? AND status=? AND user_id=?");
        ...
        if (empty($currencytid)||empty( $cardptid)||empty( $pin)||empty( $amount)){
            ...
            $vc_card_id= $cardTypeData['vc_card_id'];
                    ...
                    $active=1;
                    $checkdata =  $connect->prepare("SELECT id,max_vc_generate,creation_fee,currency,funding_percent,maintanace_fee,second_fund_fee,supplier,unload_convenience_fee,mim_bala_card_must_have,min_unload,naira_unload_exhange_rate,cardbrand,name,need_activation	 FROM  vc_type WHERE trackid=? AND status=?");
                                        ...
                                        if($amounttodeduct <=0 || !is_numeric($amounttodeduct)){
                                                ...
                                                respondBadRequest($data);
                                                   ...
                                                   // $length,$tablename,$tablecolname,$tokentag,$addnumbers,$addcapitalletters,$addsmalllletters
                                                    $jointid = createUniqueToken(7,"userwallettrans","joint_trans_tid","VC_UNLOAD",true,true,true);
                                                        ...
                                                        //    $reference=$orderId = createUniqueToken(18,"userwallettrans","orderid","VC",true,true,true);
                                                           ...
                                                           if ($addTransaction1->execute()){
                                                            ...
                                                            //   exit;
                                                                    ...
                                                                    $fundedone=false;
                                                                    if($cardsupplier==1){
                                                                       $fundedone =fundCompanyWallet($wallet_id,$amount,$narration,$reference1,$currency,$userid);
                                                                    }else if($cardsupplier==2){
                                                                         $fundedone =fundBcCompanyWallet($vc_card_id,$amount,$narration,$reference1,$currency,$userid);
                                                                    }
                                                                    if($fundedone){
                                                                         //  debit suer fund
                                                                            $funddeducted=true;//coomment this if no call back is needed for confirm
                                                                            //uncomment below if no call back is needed to confirm
                                                                            //  $funddeducted=false;
                                                                            // $iscryptotrans=0;
                                                                            // if($itsnairafund==1){
                                                                            //     $iscryptotrans=0;
                                                                            ...
                                                                            if($funddeducted){
                                                                                ...
                                                                                $userusername=$datais['username'];
                                                                                    ...
                                                                                    if($dsysresult7->num_rows>0){
                                                                                        ...
                                                                                        $response="@habnarm1\n\n*PENDING UNLOAD CARD*%0A%0AA User with following detail unloaded virtual card.%0A%0AUsername-$userusername%0AFund-`\$$amount`%0ACard Name-`$cardname`%0ACard Type-`$cardsuppliername $cardbrand`%0AOrder ID-`$reference1`%0A";
                                                                                    ...
                                                                                    $errordata = [];
                                                                                    $method=getenv('REQUEST_METHOD');
                                                                                    $text = "Card unload request in progress, expected completion within 24 hours. ";
                                                                                    $status = true;
                                                                        ...
                                                                        $hint=["Ensure to send valid data, data already registered in the database.", "Use registered API to get a valid data","Read the documentation to understand how to use this API"];
                                                                        $errordata=returnError7003($errordesc,$linktosolve,$hint);
                                                                        $text="We apologize, but the virtual card unload system is currently undergoing maintenance. Don't worry, take a deep breath, it'll be back soon. Try again later.";
                                                                        $method=getenv('REQUEST_METHOD');
                                                                        $data=returnErrorArray($text,$method,$endpoint,$errordata);
                                                                        respondBadRequest($data); 
                                                                    } 
