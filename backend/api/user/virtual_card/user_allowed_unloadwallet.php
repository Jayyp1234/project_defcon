...
if ($method == 'GET') {
    ...
    $user_pubkey = cleanme($decodedToken->usertoken);
        ...
        $query = 'SELECT allow_ngn_unload_vc,amountto_add_ngnrate FROM systemsettings WHERE id=1';
        $stmt = $connect->prepare($query);
        $stmt->execute();
        $result = $stmt->get_result();
        $row =  mysqli_fetch_assoc($result);
        $allowedngnfund=$row['allow_ngn_unload_vc'];
            ...
            if($numRow > 0){
                while($users = $result->fetch_assoc()){
                    ...
                    $coinrate=getNGNtoUSDRate($currency,$fund);
                        ...
                        $active=1;
                    $checkdata =  $connect->prepare("SELECT naira_unload_exhange_rate FROM  vc_type WHERE trackid=? AND status=?");
                  ...
                  array_push( $allResponse,array("livevalue"=>floatval($coinrate),"img"=>"https://app.cardify.co/assets/images/nigeria.png","coinrate"=>$coinrate,"coinname"=>"NGN","trackid"=>$trackid,"crypto"=>0));
                }
            }
        }
        ...
        $active=0;
        $on1=1;
        $sqlQuery = "SELECT subwallettag FROM coinproducts WHERE status=? AND can_unload_vc=? GROUP BY subwallettag";
        $stmt= $connect->prepare($sqlQuery);
        ...
        if($numRow1 > 0){
            while($users1 = $result1->fetch_assoc()){
               ...
               $walletbal=number_format((float)$walletbal, $coindecimal, '.', '');
                 $walletbal= $walletbal+0;
                $walletpendbal=number_format((float)$walletpendbal, $coindecimal, '.', '');
                ...
                // "cbusdbalance20"=>$coinbaseusd-20,"pxusdbalance"=>$paxfulusd,"cbusdbalance"=>$coinbaseusd,"livevalue"=>$getlivevalu
                  array_push( $allResponse,array("crypto"=>1,"livevalue"=>round($getlivevalu),"img"=>$imglink,"coinrate"=>$coinrate,"coinname"=>$coinname,"trackid"=>$trackid));
                    }
