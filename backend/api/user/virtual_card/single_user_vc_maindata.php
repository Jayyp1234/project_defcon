...
if ($method == 'POST') {
    ...
    $user_pubkey = cleanme($decodedToken->usertoken);
        ...
        if (!getUserWithPubKey($connect, $user_pubkey)){
            ...
            if (!isset($_POST['maintid']) || !isset($_POST['vctid'])) {
                ...
                respondBadRequest($data);
                 ...
                 $maintid = cleanme($_POST['maintid']);
                 $vctid = cleanme($_POST['vctid']);
            }
        
           if (empty($maintid)||empty($vctid)){
            ...
            $active=1;
            $mainorsubwallet=2;
            $sqlQuery = "SELECT * FROM vc_type WHERE status =? AND trackid=?";
            $stmt= $connect->prepare($sqlQuery);
            $stmt->bind_param("ss",$active,$maintid);
            $stmt->execute();
            $result= $stmt->get_result();
            ...
            if($numRow > 0){
                ...
                while($users = $result->fetch_assoc()){
                    ...
                    $sqlQuery1 = "SELECT `vc_card_id`,`vc_type_tid`,`status`, `trackid`, `balance`,`brand`, `last4`, `cvv`, `pan`, `expireMonth`, `expireyear`, `freeze`,`activated`,`cansetpin` FROM vc_customer_card WHERE user_id=? AND vc_type_tid = ? AND deleted=? AND trackid=?";
                    $stmt3= $connect->prepare($sqlQuery1);
                    $stmt3->bind_param("ssss", $userid,$trackid,$notdeleted,$vctid);
                    $stmt3->execute();
