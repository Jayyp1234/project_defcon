...
if ( $method == 'POST') {
        ...
        if ( !isset($_POST['email_noti']) || !isset($_POST['sms_noti']) || !isset($_POST['push_noti'])) {
            ...
            $email_noti = cleanme($_POST['email_noti']);
            $sms_noti = cleanme($_POST['sms_noti']);
            $push_noti =  cleanme($_POST['push_noti']);
        }
        ...
        if (empty($user_id)){
            ...
            $linktosolve="htps://";
            $hint=["Kindly pass value to the user_id, email_noti field in this register endpoint","Ensure that all data sent is not empty","Ensure that the exact data type specified in the documentation is sent."];
            $errordata=returnError7003($errordesc,$linktosolve,$hint);
            $text="Kindly pass value to the user_id, email_noti field in this register endpoint";
            $method=getenv('REQUEST_METHOD');
            $data=returnErrorArray($text,$method,$endpoint,$errordata);
            respondBadRequest($data);
        }
            ...
            if ( $num_row < 1){
                ...
                $updatePassQuery = "UPDATE users SET push_noti = ?, email_noti = ?, sms_noti = ? WHERE id = ?";
                $updateStmt = $connect->prepare($updatePassQuery);
                
                $updateStmt->bind_param('sssi',$push_noti,$email_noti,$sms_noti,$user_id);
                $updateStmt->execute();
