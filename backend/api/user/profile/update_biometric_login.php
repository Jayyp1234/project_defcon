        ...
        if(!isset($_POST['code']) ){
            ...
            if (!getUserWithPubKey($connect, $user_pubkey)){
                    ...
                    if ($dresult->num_rows == 0) {// checking if data is valid
                        ...
                        $updateStmt = $connect->prepare("UPDATE users SET activate_biometric = $status WHERE id = $user_id");
                            if ($updateStmt->execute()){
                                ...
                                $errordata=[];
                                if($status==1){
                                    $text="You Have Successfully Turned On Biometric for Login.";
                                }else{
                                    $text="You Have Successfully Turned Off Biometric for Login.";
                                }
                                $method=getenv('REQUEST_METHOD');
                                $status=true;
