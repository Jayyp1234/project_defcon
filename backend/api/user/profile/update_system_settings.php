...
if ( $method == 'POST') {
        ...
        if ( !isset($_POST['allow_deposit']) || !isset($_POST['all_withdrawal']) || !isset($_POST['allow_swap'])||!isset($_POST['display_cards']) || !isset($_POST['display_swap_options']) || !isset($_POST['display_bill_options'])) {
            ...
            $allow_deposit = cleanme($_POST['allow_deposit']);
            $all_withdrawal = cleanme($_POST['all_withdrawal']);
            $allow_swap =  cleanme($_POST['allow_swap']);
            $display_cards = cleanme($_POST['display_cards']);
            $display_swap_options = cleanme($_POST['display_swap_options']);
            $display_bill_options =  cleanme($_POST['display_bill_options']);
        }
        ...
        if (empty($user_id)){
            ...
            if ( $num_row < 1){
                ...
                $updatePassQuery = "UPDATE users SET allow_deposit = ?, all_withdrawal = ?, allow_swap= ?,display_cards = ?, display_swap_options= ?, display_bill_options= ? WHERE id = ?";
                $updateStmt = $connect->prepare($updatePassQuery);
                
                $updateStmt->bind_param('ssssssi',$allow_deposit,$all_withdrawal,$allow_swap,$display_cards,$display_swap_options,$display_bill_options,$user_id);
                $updateStmt->execute();
