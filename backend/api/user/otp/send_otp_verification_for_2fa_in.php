...
if (getenv('REQUEST_METHOD') == 'POST') {
    ...
    $isit2fasetup= isset($_POST['set2fa']) ? cleanme($_POST['set2fa']) : 1;// 1 is email 2 is phone number
        ...
        // checking if sms is sent is not older than 3 min
            ...
            if ($dresult->num_rows == 0) {// checking if data is valid
                ...
                if ($tokenStmt->execute()){
                    ...
                    if($verifytype==2){
                        ...
                        if (sendUserMail($subject,$to,$messageText, $messageHTML)) {
                            ...
                            # code...
                                ...
                                $hint=[];
                                $errordata=[];
                                $text="Check your email for 2FA code";
                                $method=getenv('REQUEST_METHOD');
                                $status=true;
                    ...
                    $errorData = returnError7003($errordesc, $linktosolve, $hint);
                    $text="Phone number not found,please complete registration";
                    $method=getenv('REQUEST_METHOD');
                    $data=returnErrorArray($text,$method,$endpoint,$errordata);
