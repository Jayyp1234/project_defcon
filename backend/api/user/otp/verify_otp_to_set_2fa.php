...
if (getenv('REQUEST_METHOD') === 'POST'){
        ...
        if(isset($_POST['identity'])) {
            $identity = cleanme($_POST['identity']);
            $token = $identity;
        }
        if(isset($_POST['type'])) {
        ...
        if((isset($_POST['code']) && empty($code)) || (isset($_POST['identity']) && empty($token))){
            ...
            if ($type == 4){
                ...
                $sql = "SELECT * FROM token WHERE useridentity =? && otp=?";
                ...
                if($result->num_rows == 1){
                    ...
                    if($time > time()){
                        ...
                        $errordata=returnError7003($errordesc,$linktosolve,$hint);
                        $text="Your Otp is expired, Click resend to get new token";
                        $method=getenv('REQUEST_METHOD');
                        $data=returnErrorArray($text,$method,$endpoint,$errordata);
                        respondBadRequest($data);
                    ...
                    //otp expired
                    $errordesc="Invalid OTP ".$identity;
                    $linktosolve="https://";
                    $hint=["Generate another token","Ensure that all data specified in the API is sent","Ensure that all data sent is not empty","Ensure that the exact data type specified in the documentation is sent."];
                    $errordata=returnError7003($errordesc,$linktosolve,$hint);
                    $text="Your Otp is invalid, Click resend to get new token";
                    $method=getenv('REQUEST_METHOD');
                    $data=returnErrorArray($text,$method,$endpoint,$errordata);
                    respondBadRequest($data);
                ...
                $getToken = $connect->prepare($sql);
                $getToken->bind_param('ss', $identity,$code);
                $getToken->execute();
                $result = $getToken->get_result();
                if($result->num_rows ==1){
                    $row = $result->fetch_assoc();
                    ...
                    if($time > time()){
                            $query="";
                            $active=1;
                            $updatePassQuery = "UPDATE users SET 2fa = $type WHERE id = $id";
