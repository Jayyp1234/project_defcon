        ...
        if(!isset($_POST['code']) ){
            ...
            if (!getUserWithPubKey($connect, $user_pubkey)){
                    ...
                    if ($dresult->num_rows == 0) {// checking if data is valid
                        ...
                        $qrCodeUrl = $ga->getQRCodeGoogleUrl('Cardify', $secret);
                        $oneCode = $ga->getCode($secret);
                        $checkResult = $ga->verifyCode($secret, $oneCode, 2); 
                        if ($oneCode == $code){
                            $updateStmt = $connect->prepare("UPDATE users SET 2fa = 1 WHERE id = $user_id");
                            if ($updateStmt->execute()){
                                ...
                                $hint=[];
                                $errordata=[];
                                $text="You Have Successfully Turned On 2FA Authentication.";
                                $method=getenv('REQUEST_METHOD');
                                $status=true;
                                ...
                                $errordesc = "Error Fetching Reset Token";
                                $linktosolve = 'https://';
                            ...
                            // send internal error response
                                $errordesc = "Incorrect Token, Try Again.";
                                $linktosolve = 'https://';
                                $hint = "500 code internal error, check ur database connections";
                                $errorData = returnError7003($errordesc, $linktosolve, $hint);
