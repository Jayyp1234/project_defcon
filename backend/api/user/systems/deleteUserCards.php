...
if ($method == 'GET') {
        ...
        $user_email = getEmailWithPubKey($connect, $user_pubkey);
        ...
        $query = "DELETE FROM `user_cards` WHERE `id` = ? AND `useremail` = ?";
        $delete = $connect->prepare($query);
        $delete->bind_param("ss",$cardid,$user_email);
        $delete->execute();
        if ( $delete->affected_rows>0 ){
            ...
            $errordesc = "";
            $linktosolve = "https://";
            $hint = "Card has been Deleted in the database";
            $errordata = [];
