...
if ($method == 'POST') {
    ...
    $user_pubkey = cleanme($decodedToken->usertoken);
       ...
       if (isset ($_POST['hascashback'])&&!empty(trim($_POST['hascashback']))) {
            $queryClause .= " AND average_cashback_percent> ?";
            $bindparams[]=0;
            $bindparamss.="i";
        }
        ...
        $sqlQuery = "SELECT MAX(image) AS image,MAX(generalname) AS generalname,MAX(average_cashback_percent) AS average_cashback_percent,groupname  FROM bill_top_up_main_products  WHERE status=?  $queryClause  GROUP BY 	groupname $limitclause";
