...
if ($method == 'POST') {
    ...
    $user_pubkey = cleanme($decodedToken->usertoken);
        ...
        if (!isset($_POST['voucher_tid'])||strlen($_POST['voucher_tid'])<2) {
        ...
        // get minimum price
        $active=1;
        $getdataemail =  $connect->prepare("SELECT amount,MAX(amount_to_pay) as amount_to_pay FROM bill_voucher_prices WHERE voucher_tid=? AND status=? GROUP BY amount");
        ...
        if( $getresultemail->num_rows> 0){
                while($getthedata= $getresultemail->fetch_assoc()){
                    array_push($allResponse,json_decode(json_encode($getthedata), true));
                }
